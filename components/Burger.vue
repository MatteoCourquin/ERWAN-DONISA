<template>
  <header @click="isActive = !isActive"
    :class="['z-[900] fixed top-14 right-insetMain flex justify-center items-center cursor-pointer shadow-inner border bg-white rounded-full w-20 h-20 transition-all', isScrolled || isActive ? 'flex opacity-100 scale-1' : 'flex opacity-100 scale-0 sm:opacity-0 sm:invisible']">
    <div
      :class="['w-8 h-[13px] flex flex-col items-center justify-between burger-menu-container', isActive && 'active-burger']">
      <div :class="['transition-all duration-300 rounded-full w-full h-[2px] burger-menu-item backdrop-filter']">
      </div>
      <div :class="['transition-all duration-300 rounded-full w-full h-[2px] burger-menu-item backdrop-filter']">
      </div>
    </div>
  ...
  <nav
    :class="['menu flex flex-col justify-center items-center gap-10 pb-16 fixed transition-all duration-300 top-0 w-screen h-screen z-[800] bg-black', isActive ? 'translate-y-0' : '-translate-y-full']">
    <p @click="changeLanguage" class='absolute top-14 left-insetMain link link_underline transition-all link_white'
      to="/">{{ language }}</p>
    <ul>
      <li>
